...
public class HandEvaluator {
    ...
    private int[] rankDist = new int[4];
    
    /** The suit distribution (number of cards for each suit). */
    private int[] suitDist = new int[13];
            ...
            if (suitDist[i] >= 5) {
                ...
                for (Card card : cards) {
                    if (card.getSuit() == flushSuit) {
                        if (!wheelingAce || card.getPower() != 1) {
                            flushRank = card.getPower();
                            break;
                        }
                    }
                }
    ...
    private void findStraight() {
        ...
        if ((count == 4) && (rank == 5) && (rankDist[1] > 0)) {
            wheelingAce = true;
            straightRank = rank;
        }
    }
