...
/**
 * Yoinked from http://pastebin.com/index/f355d003d
 * 
 * A single text field supporting text entry
 *
 * @author kevin
 */
public class PasswordTextField extends AbstractComponent {
    ...
    protected void doPaste(String text) {
        ...
        for (int i=0;i<text.length();i++) {
            keyPressed(-1, text.charAt(i), true);
        }
        if (old != value)
            textChanged();
    }
    ...
    public void keyPressed(int key, char c) {
        keyPressed(key, c, false);
    }
    
    /**
     * @see org.newdawn.slick.gui.AbstractComponent#keyPressed(int, char)
     */
    public void keyPressed(int key, char c, boolean pasting) {
        if (hasFocus()) {
